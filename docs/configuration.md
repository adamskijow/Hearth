...
Hearth refuses to apply a malformed config or one with an error-severity
diagnostic; an unknown runner or mode never falls through to an active default.
...
| `probeModel` | string or null | `null` | Optional deep readiness probe. The default `/api/version` probe only proves the HTTP server answers; it misses a wedged model runner that still responds there. Set a model name and, while that model is already resident from real use, Hearth periodically runs a one-token generation against it. Without observed client traffic through the metrics proxy, repeated failures alert but withhold restart because a long queued generation is indistinguishable from a wedge. Off by default; it names a model and does GPU work. Preferences can explicitly load and test the model once before saving. |
...
| `deepProbeTimeoutSeconds` | number | `30` | How long an inference probe against an already-resident model may take before it counts as wedged. Scheduled checks never cold-load an idle model. Explicit setup and Check Now tests give a cold or uncertain load at least 60 seconds and cannot turn that load timeout into a runner outage. Floored at 1. |
...
| `notificationsPaused` | bool | `false` | Vacation mode: silence every channel (local, ntfy, webhook) without touching their settings. Events are still logged. Also togglable from the menu (Pause Notifications). During normal operation, one outage sends one down alert even if multiple recovery attempts fail; the event log still records each retry. |
