...
| `deepProbeTimeoutSeconds` | number | `30` | How long an inference probe against an already-resident model may take before it counts as wedged. Cold or uncertain model loads receive at least 60 seconds, because cancelling Ollama during loading aborts the load and can otherwise create a false restart loop. Floored at 1. |
