...
| `deepProbeTimeoutSeconds` | number | `30` | How long an inference probe against an already-resident model may take before it counts as wedged. Cold or uncertain model loads receive at least 60 seconds and cannot trigger recovery, because cancelling Ollama during loading aborts the load and does not prove serving inference is wedged. Floored at 1. |
